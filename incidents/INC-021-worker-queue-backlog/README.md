...
name             messages\_ready  messages\_unacknowledged  consumers
...
The worker container was absent from the running container list while RabbitMQ remained healthy.



This isolated the failure to the consumer/worker layer rather than the message broker.
...
RabbitMQ remained running and healthy.



This ruled out a broker outage.
...
The worker container was not running.



This explained why no consumer was attached to the queue.
...
The queue contained 114 ready messages with:



\* zero unacknowledged messages

\* zero consumers



This indicated messages were waiting safely in RabbitMQ and had not been delivered to a worker.
...
Because messages were present in the queue, producers were still capable of publishing work.



The failure was therefore isolated to the consumer side.
...
The NIGHTWATCH worker process was not running.



With no active consumer connected to RabbitMQ, newly published jobs accumulated in the `nightwatch-jobs` queue.
...
The backlog drained completely and one active consumer remained connected.



This confirmed successful worker recovery and queue processing.
...
\* Alert on sustained growth of `messages\_ready`.
