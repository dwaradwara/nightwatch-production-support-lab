...
The test was isolated from NIGHTWATCH production-style data volumes so the actual lab environment was not put at risk.
...
This distinguished storage exhaustion from:
...
The incident used a temporary constrained `tmpfs` rather than filling NIGHTWATCH's real Docker storage.



This allowed realistic ENOSPC behavior to be reproduced without risking application or database data.
...
The temporary filesystem exhausted its available capacity.



Once no additional blocks were available, further writes failed with an ENOSPC condition.
...
This confirmed storage capacity and write functionality were restored.
...
\* Production-style storage incident reasoning
